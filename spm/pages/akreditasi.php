...
<head>
  ...
  <title>SI Akreditasi | Akreditasi</title>
      ...
      <div class="container-fluid">
        ...
        <div class="card">
          ...
          <div class="card-header">
            <h1 class="card-title"><b>Akreditasi Program Studi LAM Teknik</b></h1>
            <a href="akreditasi/add_teknik.php">
          ...
          <div class="card-body">
            <table id="teknik" class="table table-bordered table-striped">
              ...
              <?php
                include 'koneksi.php';
                $stid = oci_parse($koneksi, 'SELECT * from akreditasi
                ...
                INNER JOIN prodi_teknik ON prodi_teknik.id_program = akreditasi.id_program
                AND prodi_teknik.id_jurusan = akreditasi.id_jurusan
              ...
              ?>
                ...
                <td> 
                  <a href="<?= $data[6]; ?>" target="_blank" rel="noopener noreferrer">
                    <?php echo $data[7]; ?>
                  </a>
                </td>
                <td class="td-actions">
                  <div class="btn-group btn-sm" role="group" aria-label="Basic example">
                    <a href="akreditasi/edit_teknik.php?id=<?= $data[0] ?>">
                      <button type="button" class="btn-sm btn-success">
                        <i class="nav-icon fas fa-pen"></i>
                      </button>
                    </a>
                    <a href="akreditasi/delete_teknik.php?id=<?= $data[0] ?>" 
        ...
        <div class="card">
          <!-- /.card-header -->
          <div class="card-header">
            <h1 class="card-title"><b>Akreditasi Program Studi LAM Infokom</b></h1>
            <a href="akreditasi/add_infokom.php">
              <button type="button" class="btn-sm btn-info float-sm-right">
                Add New
              </button>
            </a>
          ...
          <div class="card-body">
            <table id="infokom" class="table table-bordered table-striped">
              ...
              <?php
                ...
                INNER JOIN jurusan ON jurusan.nomor = akreditasi.id_jurusan
                INNER JOIN peringkat ON peringkat.id_peringkat = akreditasi.id_peringkat
                INNER JOIN prodi_infokom ON prodi_infokom.id_program = akreditasi.id_program
                AND prodi_infokom.id_jurusan = akreditasi.id_jurusan
                ORDER BY kadaluwarsa ASC');
                oci_execute($stid);
                while ($data = oci_fetch_row($stid)) {
              ?>
                ...
                <td class="td-actions">
                  <div class="btn-group btn-sm" role="group" aria-label="Basic example">
                    <a href="akreditasi/edit_infokom.php?id=<?= $data[0] ?>">
                      <button type="button" class="btn-sm btn-success">
                        <i class="nav-icon fas fa-pen"></i>
                      </button>
                    </a>
                    <a href="akreditasi/delete_infokom.php?id=<?= $data[0] ?>" 
							        onclick="return confirm('Anda yakin akan menghapus data ini?')">
                      <button type="button" class="btn-sm btn-danger" data-toggle="modal" data-target="hashtagModalCenter1">
                      <i class="nav-icon fas fa-trash"></i>
                      </button>
