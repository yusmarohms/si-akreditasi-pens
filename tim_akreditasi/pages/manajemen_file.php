      ...
      <div class="container-fluid">
        ...
        <div class="card">
          ...
          <div class="card-header">
            <h3 class="card-title"><b>File Akreditasi LAM Teknik</b></h3>
              <?php
                include 'koneksi.php';
                $stid = oci_parse($koneksi, "SELECT COUNT (*) from prodi_teknik
                INNER JOIN program ON program.nomor = prodi_teknik.id_program
                INNER JOIN jurusan ON jurusan.nomor = prodi_teknik.id_jurusan
                INNER JOIN tim_akreditasi ON tim_akreditasi.id_program = prodi_teknik.id_program 
                AND tim_akreditasi.id_jurusan = prodi_teknik.id_jurusan
                where username='yusma@gmail.com' ");
                oci_execute($stid);
                while ($data = oci_fetch_row($stid)) {
                  if ($data[0] > 0) {
                    echo '<a href="manajemen_file/add_teknik.php">
                    <button type="button" class="btn-sm btn-info float-sm-right">
                    Add New
                    </button></a>';  
                  } else if ($data[0] == 0){
                    echo '
                    <button type="button" class="btn-sm btn-danger float-sm-right">
                    No Data
                    </button>';
                  }
                }
          ...
          <div class="card-body">
            <table id="teknik" class="table table-bordered table-striped">
              ...
              <?php
                ...
                INNER JOIN tim_akreditasi ON tim_akreditasi.id_program = file_teknik.id_program 
                AND tim_akreditasi.id_jurusan = file_teknik.id_jurusan
                where username='yusma@gmail.com' ");
                oci_execute($stid);
                while ($data = oci_fetch_row($stid)) {
              ?>
              <tr>
                ...
                <td class="td-actions">
                  ...
                  <a href="manajemen_file/edit_teknik.php?id=<?= $data[0] ?>">
                    ...
                    <a href="manajemen_file/delete_teknik.php?id=<?= $data[0] ?>" 
        ...
        <div class="card">
          ...
          <div class="card-header">
            ...
            <?php
                include 'koneksi.php';
                $stid = oci_parse($koneksi, "SELECT COUNT (*) from prodi_infokom
                INNER JOIN program ON program.nomor = prodi_infokom.id_program
                INNER JOIN jurusan ON jurusan.nomor = prodi_infokom.id_jurusan
                INNER JOIN tim_akreditasi ON tim_akreditasi.id_program = prodi_infokom.id_program 
                AND tim_akreditasi.id_jurusan = prodi_infokom.id_jurusan
                where username='yusma@gmail.com' ");
                oci_execute($stid);
                while ($data = oci_fetch_row($stid)) {
                  if ($data[0] > 0) {
                    echo '<a href="manajemen_file/add_infokom.php">
                    <button type="button" class="btn-sm btn-info float-sm-right">
                    Add New
                    </button></a>';  
                  } else if ($data[0] == 0){
          ...
          <div class="card-body">
            <table id="infokom" class="table table-bordered table-striped">
              ...
              <?php
                ...
                INNER JOIN tim_akreditasi ON tim_akreditasi.id_program = file_infokom.id_program 
                AND tim_akreditasi.id_jurusan = file_infokom.id_jurusan
                where username='yusma@gmail.com' ");
                oci_execute($stid);
                while ($data = oci_fetch_row($stid)) {
              ?>
              <tr>
                ...
                <td><a href="<?= $data[6]; ?>" target="_blank" rel="noopener noreferrer"><?= $data[6]; ?></a></td>
                <td><a href="<?= $data[5]; ?>" target="_blank" rel="noopener noreferrer"><?= $data[5]; ?></a></td>
                <td><a href="<?= $data[3]; ?>" target="_blank" rel="noopener noreferrer"><?= $data[3]; ?></a></td>
                <td><a href="<?= $data[4]; ?>" target="_blank" rel="noopener noreferrer"><?= $data[4]; ?></a></td>
                <td><a href="<?= $data[7]; ?>" target="_blank" rel="noopener noreferrer"><?= $data[7]; ?></a></td>
                <td class="td-actions">
                  ...
                  <a href="manajemen_file/edit_infokom.php?id=<?= $data[0] ?>">
                    ...
                    <a href="manajemen_file/delete_infokom.php?id=<?= $data[0] ?>" 
							        onclick="return confirm('Anda yakin akan menghapus data ini?')">
                      <button type="button" class="btn-sm btn-danger">
                      <i class="nav-icon fas fa-trash"></i>
                      </button>
